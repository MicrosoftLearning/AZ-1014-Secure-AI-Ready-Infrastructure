---
lab:
  title: 'AI Ready Security hands-on exercise: Implement customer-managed keys (CMK) for Microsoft Foundry'
  description: Key rotation can be manual or policy-based in Key Vault. When a new key version is created, the Foundry resource must be updated to reference it. Rotation does not re-encrypt existing data, and CMK-enabled resources cannot revert to Microsoft-managed keys.
  duration: 25 minutes
  level: 400
  islab: true
  primarytopics:
