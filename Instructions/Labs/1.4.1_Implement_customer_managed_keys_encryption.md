---
lab:
  title: 'AI Ready Security hands-on exercise: Implement customer-managed keys (CMK)
    for Microsoft Foundry'
  description: All Microsoft Foundry resources are encrypted at rest by default with
    Microsoft-managed keys. Organizations that must control the lifecycle of their
    encryption keys can instead enable customer-managed keys (CMKs). With CMKs, encryption
    and decryption operations use a key stored in Azure Key Vault, allowing the organization
    to define its own key-creation, rotation, and deletion policies and to satisfy
    compliance frameworks such as ISO 27001 or SOC 2.
  duration: 25 minutes
  level: 400
  islab: true
  primarytopics:
...
# AI Ready Security hands-on exercise: Implement customer-managed keys (CMK) for Microsoft Foundry
...
Microsoft Foundry accesses the Key Vault key through a managed identity - either system-assigned (tied to the Foundry resource) or user-assigned (centrally managed or shared). This identity removes the need for stored credentials and is granted only the minimal permissions required to perform encryption:
- get  
- wrapKey  
- unwrapKey  
...
- **Permissions**: To create Microsoft Foundry resources, you should have the Owner role assigned at the Azure subscription.
...
1. On the **Basics** tab of the **Create a key vault** page, specify the following settings (leave others with their default values) and select **Next**:
   ...
   |Region|The name of the Azure region where you intend to create the Microsoft Foundry resource|
...
### Task 3: Create a Microsoft Foundry resource

1. In the web browser displaying the **Keys** page of the Azure Key Vault instance, use the **Search** text box at the top of the page to search for **Microsoft Foundry** and, in the list of results, select **Microsoft Foundry**.
...
1. On the **Network** tab of the **Create a Foundry resource** page, ensure that the option **All networks, including the internet can access this resource** is selected and then select **Next**.
...
1. On the **Encryption** tab, accept the default settings (encryption using Microsoft-managed keys) and select **Next**.
   ...
   **Note**: You will use the managed identity of the Foundry resource to access the key you generated in Azure Key Vault, so you have to create the resource first with encryption using a Microsoft-managed key.
...
### Task 4: Grant the Microsoft Foundry managed identity access to Azure Key Vault
...
1. On the **Role** tab of the **Add role assignment** page, under the **Job function roles** listing, in the **Search** text box, enter **Key Vault Crypto Service Encryption User**, in the list of search results, select **Key Vault Crypto Service Encryption User**, and select **Next**.
   ...
   **Note**: Microsoft Foundry resource system-assigned managed identity requires **get**, **wrapKey**, **unwrapKey** key permissions. The **Key Vault Crypto Service Encryption User** role is the least-privileged built-in role that grants these permissions.
...
1. On the **Select members** pane, in the **Managed identity** drop-down list, select **Microsoft Foundry** entry, in the list of search results, select **aifoundry-cmkencryption-resource**, and then click **Select**.
1. Back on the **Members** tab of the **Add role assignment** page, select **Next**.
1. On the **Conditions** tab of the **Add role assignment** page, select **Review + assign**.
1. On the **Review + assign** tab, select **Review + assign**.

### Task 5: Configure the Microsoft Foundry resource to use the customer-managed encryption key

1. In the web browser displaying the **aifoundry-encryption-key** page, use the **Search** text box at the top of the page to search for **Microsoft Foundry** and, in the list of results, select **Microsoft Foundry**.
1. On the **Microsoft Foundry** page, in the vertical menu on the left side, select **Use with Foundry** and then select **Foundry**.
1. On the **Microsoft Foundry \| Foundry** page, select **aifoundry-cmkencryption-resource**.
