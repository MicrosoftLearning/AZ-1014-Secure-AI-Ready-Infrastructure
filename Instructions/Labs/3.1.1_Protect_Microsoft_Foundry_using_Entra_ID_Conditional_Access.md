---
lab:
  title: 'AI Ready Security hands-on exercise: Protect Microsoft Foundry by using
    Microsoft Entra ID Conditional Access'
  description: Modern security extends beyond the network perimeter and relies on
    identity-driven access control. Microsoft Entra Conditional Access evaluates multiple
    signals to make access decisions for users, devices, and applications. Signals
    include, for example, user's identity and group membership, device state and compliance,
    IP location, target application, and real-time risk assessment. Policies can combine
    these signals to implement complex access rules, such as requiring multifactor
    authentication in combination with restricting connections from trusted networks
...
# AI Ready Security hands-on exercise: Protect Microsoft Foundry by using Microsoft Entra ID Conditional Access
...
Conditional Access policies can be applied to protect Microsoft Foundry Foundry resources. For example, administrators can assign policies to the following apps by their App IDs:
...
- In the **Group name** text box, enter **CA - Microsoft Foundry Users**.
- In the **Group description** text box, enter **Users allowed to access Microsoft Foundry (subject to CA constraints)**.
...
### Task 3: Create a Microsoft Entra Conditional Access policy that allows access to Microsoft Foundry portal
...
- In the **Name** text box, enter **Allowing access to Microsoft Foundry**.
...
- On the **Select users and groups** pane, in the **Search** text box, enter **CA - Microsoft Foundry Users**, in the list of results, select **CA - Microsoft Foundry Users**, and then click **Select**.
...
- On the **Select** pane, in the **Search** text box, enter **Microsoft Foundry App**, select **Microsoft Foundry App** in the list of results, and then click **Select**.

   **Note**: In case **Microsoft Foundry App** does not appear in the list of results, run the following script from the PowerShell pane of Azure Cloud Shell:
   ...
   if (-not $spExists) {
      ...
      $body = @{
         appId = $foundryAppId
         displayName = "Microsoft Foundry App"
      } | ConvertTo-Json
      ...
      Write-Host -ForegroundColor Green 'Microsoft Foundry App service principal created successfully.'
   } else {
      Write-Host -ForegroundColor Green 'Microsoft Foundry App service principal already exists.'
   }
   ```
...
### Task 4: Create a Microsoft Entra Conditional Access policy that blocks access to Microsoft Foundry portal

1. Back on the **Conditional Access \| Policies** page, select **+ New policy**.
1. On the **New** page, perform the following actions:

- In the **Name** text box, enter **Blocking access to Microsoft Foundry**.
...
1. On the **Conditional Access \| Policies** page, in the **Search** text box, enter **Allowing access to Microsoft Foundry** and, in the list of results, select **Allowing access to Microsoft Foundry**.
1. On the **Allowing access to Microsoft Foundry** page, select **Delete** and, when prompted for the confirmation, select **Yes**.
1. Back on the **Conditional Access \| Policies** page, in the **Search** text box, enter **Blocking access to Microsoft Foundry** and, in the list of results, select **Blocking access to Microsoft Foundry**.
1. On the **Blocking access to Microsoft Foundry** page, select **Delete** and, when prompted for the confirmation, select **Yes**.
...
1. On the **Groups \| All groups** page, in the **Search** text box, enter **CA - Microsoft Foundry Users** and, in the list of results, select **CA - Microsoft Foundry Users**.
1. On the **CA - Microsoft Foundry Users** page, select **Delete** and, when prompted for the confirmation, select **OK**.