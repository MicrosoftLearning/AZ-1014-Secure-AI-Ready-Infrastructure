---
lab:
  title: 'AI Ready Security hands-on exercise: Enhance security of Microsoft Foundry
    compute resources by using Azure Policy'
  description: Azure Policy is a governance tool that allows organizations to enforce
    standards and ensure that Azure resources remain compliant with defined policies.
    It provides a framework to define rules and effects that can be applied at the
    subscription, resource group, or individual resource level. Policies are automatically
    enforced across all resources within their scope, helping maintain consistent
    security, operational, and compliance standards.
  duration: 15 minutes
  level: 300
  islab: true
  primarytopics:
