---
lab:
  title: 'AI Ready Security hands-on exercise: Configure authorization for Microsoft
    Foundry project'
  description: Access management for cloud resources is a critical function for any
    organization that is using the cloud. Azure role-based access control (Azure RBAC)
    helps manage who has access to Azure resources, what they can do with those resources,
    and what areas they have access to. Azure RBAC is an authorization system built
    on Azure Resource Manager that provides fine-grained access management to Azure
    resources.
  duration: 10 minutes
  level: 500
  islab: true
...
# AI Ready Security hands-on exercise: Configure authorization for Microsoft Foundry project
...
Within Microsoft Foundry, RBAC provides consistent, role-based authorization across both the account and its projects.
...
Microsoft Foundry includes built-in roles that govern access at both levels:
- Azure AI User – Grants read-only access to Foundry resources and projects, including permissions to view Foundry assets and perform limited data actions.
...
### Task 1: Create a Microsoft Foundry resource and the default project
...
1. On the **AI Foundry** page, in the vertical menu on the left side, select **Use with AI Foundry** and then select **Microsoft Foundry**.
1. On the **AI Foundry \| Microsoft Foundry** page, select **+ Create**.
1. On the **Basics** tab of the **Create an Microsoft Foundry resource** page, specify the following settings (leave others with their default values) and select **Next**:
   ...
   |Region|The name of the Azure region where you intend to create Microsoft Foundry project|
   |Default project name|**rbacproject01**|

1. On the **Network** tab of the **Create an Microsoft Foundry resource** page, ensure that the option **All networks, including the internet can access this resource** is selected and then select **Next**.
...
### Task 2: Create an additional Microsoft Foundry project

1. In the web browser displaying the Microsoft Foundry resource's deployment status page in the Azure portal, select **Go to resource** to navigate to the project's **Overview** page.
