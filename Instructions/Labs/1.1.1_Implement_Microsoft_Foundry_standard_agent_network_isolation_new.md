...
lab:
  ...
  description: Provisioning standard environments is not available directly from the Microsoft Foundry portal, but you can implement them either programmatically or via deployment templates. Sample Azure Resource Manager and Bicep templates are available from the foundry-samples GitHub repository. They allow you to automatically create a Foundry account and project, deploy a model, and configure authentication using Managed Identity. With the Standard configuration option, you can deploy a network-secured Azure AI agent environment with private networking and role-based access control (RBAC).
  duration: 20 minutes
  level: 500
  islab: true
  primarytopics:
