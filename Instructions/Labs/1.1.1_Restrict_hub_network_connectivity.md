---
lab:
  title: 'AI Ready Security hands-on exercise: Configure Microsoft Foundry hub network
    connectivity by using a managed network'
  description: Securing network connectivity for Microsoft Foundry hubs is essential
    for maintaining data confidentiality, preventing unauthorized access, and complying
    with enterprise security policies. Microsoft Foundry provides built-in support
    for network isolation, enabling you to control both inbound and outbound traffic
    to the hub and its associated resources. This isolation ensures that compute instances,
    managed endpoints, and other AI workloads operate within a tightly controlled
    network boundary while maintaining connectivity to approved Azure services.
  duration: 25 minutes
  level: 500
  islab: true
  primarytopics:
...
# AI Ready Security hands-on exercise: Configure Microsoft Foundry hub network connectivity by using a managed network
...
When configuring a hub-based Microsoft Foundry environment, there are two primary aspects of network isolation to address:
...
2. Outbound access restrictions control how resources within the hub (such as compute instances, managed online endpoints, and serverless jobs) access external services. Outbound isolation is achieved through a managed virtual network (Managed VNet) that Microsoft Foundry automatically creates and maintains for the hub and all associated projects. The managed VNet ensures compute resources operate in a restricted network environment and that outbound traffic is routed only to explicitly approved destinations.
...
**Note**: The exercise involves reviewing inbound and outbound access settings of an Microsoft Foundry hub without the actual implementation of its configuration options. This is intentional in order to minimize the duration and cost of the implementation. Microsoft Foundry defers creating the managed virtual network until a compute resource is created. With automatic creation, it can take about 30 minutes to create the first compute resource because it also provisions the network. In addition, adding outbound FQDN rules in the allow only approved outbound mode triggers deployment of Azure Firewall, with the corresponding charges added to your Azure subscription bill. 

### Task 1: Create a virtual network to host resources that will be part of the Microsoft Foundry hub environment

**Note**: The virtual network you are creating in this task is intended for connecting to Microsoft Foundry hub and for hosting resources that Microsoft Foundry hub compute resources will be connecting to.
...
1. On the **Basics** tab of the **Create virtual network** page, specify the following settings and select **Next**:
   ...
   |Region|The name of the Azure region where you want to deploy the Microsoft Foundry hub and its resources|
...
1. In the web browser displaying the deployment page of the Azure virtual network, use the **Search** text box at the top of the page to search for **Microsoft Foundry** and, in the list of results, select **Microsoft Foundry**.
1. On the **Microsoft Foundry** page, in the vertical menu on the left side, select **Use with Foundry** and then select **Foundry**.
1. On the **Microsoft Foundry \| Foundry** page, select **+ Create**.
1. On the **Basics** tab of the **Create a Foundry resource** page, specify the following settings (leave others with their default values) and select **Next**:
...
1. On the **Network** tab of the **Create a Foundry resource** page, perform the following tasks:
...
### Task 6: Review inbound access configuration of an Microsoft Foundry hub

1. In the web browser displaying the Azure Container Registry instance deployment page, use the **Search** text box at the top of the page to search for **Microsoft Foundry** and, in the list of results, select **Microsoft Foundry**.
...
1. On the **Basics** tab of the **Azure AI hub** page, specify the following settings (leave others with their default values) and select **Next: Storage**:
   ...
   |Subscription|The name of the Azure subscription you are using in this exercise|
   |Resource group|**aifoundry-isolated-RG**|
   |Region|The name of the Azure region where you intend to create Microsoft Foundry hub|
...
**Note**: As mentioned earlier, this is intended in order to minimize the duration and cost of this exercise. Microsoft Foundry defers creating the managed virtual network until a compute resource is created. With automatic creation, it can take about 30 minutes to create the first compute resource because it also provisions the network. In addition, adding outbound FQDN rules in the allow only approved outbound mode triggers deployment of Azure Firewall, with the corresponding charges added to your Azure subscription bill. 
